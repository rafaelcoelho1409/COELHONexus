"""YouTube Content Search — 3-step wizard (Source → Ingest → Ask).

Step 1 (Source) ships first: a 4-mode tab strip (Search / Videos /
Playlist / Channel) backed by /api/v1/youtube/{search,videos,playlist,
channel}. All four modes converge on the same results grid + selection
cart. Steps 2 (Ingest) and 3 (Ask) own their own bodies — Ask will host
the grounded-Q&A pipeline that used to live as a single-shot form here.

Client-side dynamic logic lives in static/js/youtube_content_search.js.
"""
from fasthtml.common import (
    Button, Div, Input, Option, P, Script, Select, Span, Textarea,
)
