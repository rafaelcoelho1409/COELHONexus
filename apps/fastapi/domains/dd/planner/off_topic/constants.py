...
_JUDGE_HEAD_CHARS = 3000     # leading chars (covers TOC + first paragraph)
_JUDGE_TAIL_CHARS = 1500     # trailing chars (covers license/footer/edit-links)
...
_JUDGE_BODY_MIN_FOR_SPLIT = _JUDGE_HEAD_CHARS + _JUDGE_TAIL_CHARS + len(_JUDGE_HEAD_TAIL_SEP)

# Legacy alias kept for backwards-compat; equals the combined head+tail
# size so any caller using `_JUDGE_BODY_CHARS` as a sanity cap still sees
# the right ceiling.
_JUDGE_BODY_CHARS = _JUDGE_BODY_MIN_FOR_SPLIT
