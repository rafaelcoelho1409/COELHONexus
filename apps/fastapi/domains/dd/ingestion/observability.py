...
def record_ingestion_run(
    *,
    ...
    try:
        if (inst := infra.otel.service.get_instrument("ingestion_run_total")) is not None:
            inst.add(1, attributes = attrs)
        if duration_s is not None and (
            inst := infra.otel.service.get_instrument("ingestion_run_duration")
        ) is not None:
            inst.record(max(duration_s, 0.0), attributes = attrs)
        if output_files is not None and (
            inst := infra.otel.service.get_instrument("ingestion_output_files")
        ) is not None:
            inst.record(max(output_files, 0), attributes = attrs)
        if output_bytes is not None and (
            inst := infra.otel.service.get_instrument("ingestion_output_bytes")
        ) is not None:
