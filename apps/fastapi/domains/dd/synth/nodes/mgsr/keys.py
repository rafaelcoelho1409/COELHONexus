"""mgsr — MinIO key builders."""
from __future__ import annotations
from . import params



def versioned_blob_key(slug: str, chapter_id: str, manifest_hash: str) -> str:
    return f"{params.BLOB_PREFIX}/{slug}/{chapter_id}/mgsr/{manifest_hash}.json"


def latest_blob_key(slug: str, chapter_id: str) -> str:
    return f"{params.BLOB_PREFIX}/{slug}/{chapter_id}/mgsr-latest.json"


def checklist_latest_key(slug: str, chapter_id: str) -> str:
    return f"{params.BLOB_PREFIX}/{slug}/{chapter_id}/checklist-latest.json"


def outline_latest_key(slug: str, chapter_id: str) -> str:
    return f"{params.BLOB_PREFIX}/{slug}/{chapter_id}/outline-latest.json"
