...
import domains
from . import domain, keys, prompts, schemas, versions
...
_DIGEST_RESPONSE_FORMAT = {
    ...
    "json_schema": {
        "name":   "source_digest",
        "schema": schemas.LLMDigestPayload.model_json_schema(),
        "strict": False,
    },
}
...
async def _digest_one_source(
    *,
    ...
    source_key: str,
    source_md: str,
) -> tuple[Optional[schemas.SourceDigest], Optional[str]]:
    ...
    async with sem:
        t0 = time.monotonic()
        source_vault_hashes = domain.extract_vault_hashes(source_md)
        valid_hash_set = set(source_vault_hashes)

        def _build_draft_prompt(char_cap: int) -> str:
            return prompts.build_digest_prompt(
        ...
        issues = domain.validate_source_digest(
            payload,
            ...
            while attempt < _MAX_REPAIR_ATTEMPTS and issues:
                ...
                try:
                    ...
                    if new_payload is None:
                        break
                    new_issues = domain.validate_source_digest(
                        new_payload,
                        valid_section_ids = valid_section_ids,
                        valid_vault_hashes = valid_hash_set,
                    )
        ...
        if (
            ...
            payload.source_title = domain.derive_source_title_fallback(
        ...
        wall_ms = int((time.monotonic() - t0) * 1000)
        src_digest = schemas.SourceDigest(
            source_key = source_key,
...
async def digest_construct_run(state: domains.dd.synth.state.SynthState) -> dict:
    ...
    for sk, raw_body in zip(source_keys, bodies):
        ...
        if "<code-ref hash=" in body_text:
            ...
            continue
        try:
            sentinelized, entries = domains.dd.synth.nodes.vault.domain.sentinelize_doc(body_text)
    ...
    for _, body in pairs:
        all_vault_hashes.update(domain.extract_vault_hashes(body))
    ...
    per_source: list[schemas.SourceDigest] = [d for d, _ in results if d is not None]
    ...
    per_source, merged_sections = domain.merge_overlapping_sections(
    ...
    per_section = domain.build_per_section_index(per_source, section_ids)
    coverage = domain.compute_coverage_stats(
    ...
    chapter_digest = schemas.ChapterDigest(
        chapter_id = chapter_id,
        chapter_title = chapter_title,
        framework_slug = slug,
        n_pydantic_fail = n_pydantic_fail,
    ...
    stats = {
        ...
        "manifest_hash":        manifest_hash,
        "cache_hit":            False,
        "prompt_version":       versions.DIGEST_PROMPT_VERSION,
    }
    await domains.dd.synth.runtime.progress.service.emit_progress(
        thread_id, "digest_construct", "done",
        n_sources = stats["n_sources"],
