...
def event_channel(scan_id: str) -> str:
    ...
    return f"coelhonexus:rr:{scan_id}:events"
...
def snapshot_key(scan_id: str) -> str:
    ...
    return f"coelhonexus:rr:{scan_id}:snapshot"
...
def task_id_key(scan_id: str) -> str:
    ...
    return f"coelhonexus:rr:{scan_id}:task_id"
...
def extraction_cache_key(arxiv_id: str) -> str:
    ...
    return f"coelhonexus:rr:cache:extraction:{params.EXTRACTION_PROMPT_VERSION}:{aid}"
...
def fs_mirror_key(scan_id: str, path: str) -> str:
    ...
    return f"coelhonexus:rr:{scan_id}:fs:{path}"
...
def fs_mirror_index_key(scan_id: str) -> str:
    ...
    return f"coelhonexus:rr:{scan_id}:fs:index"
...
def code_synth_status_key(scan_id: str, arxiv_id: str, prompt_version: str) -> str:
    ...
    return f"coelhonexus:rr:{scan_id}:code_synth:{prompt_version}:{arxiv_id}:status"
