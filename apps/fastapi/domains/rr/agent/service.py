"""I/O orchestration for the RR agent — Imperative Shell.

MCP client adapter: wraps `langchain-mcp-adapters.MultiServerMCPClient`
as a process-wide singleton pointed at the in-cluster FastMCP server.
Returns LangChain `BaseTool` objects that DeepAgents subagents can hold
in their `tools=` list.
...
def _mcp_url() -> str:
    ...
    raw = os.environ.get(keys.MCP_URL_ENV, "").strip()
    return raw or keys.MCP_URL_DEFAULT
