...
def _mirror(scan_id: str, path: str, value: Any) -> None:
    ...
    try:
        runtime.service.mirror_write_sync(scan_id, path, value)
...
def _safe_emit(scan_id: str, phase: str, message: str) -> None:
    ...
    try:
        runtime.service.emit_event_sync(scan_id, phase, message=message)
    except Exception as e:
...
def stash_discovery_result(
    ...
    Args:
        ...
        source: One of 'arxiv', 'semantic_scholar', 'huggingface_daily_papers',
            'hn', 'openalex'. Must match the subagent's source.
    """
    ...
    # only ever read `.content`, and `domain.parse_tool_message_content`
    # silently turns any unparseable string (including an error message)
    # into `[]`. A genuine infra failure was indistinguishable from
    # "arxiv legitimately found nothing." Check `.status` FIRST so a real
    # failure surfaces as an error instead of a clean empty stash.
    ...
    try: runtime.llm_counter.service.set_phase("discovery")
...
def write_extraction(
    ...
    path = keys.fs_extraction_path(arxiv_id)
    # writing an arxiv_id not in top_n (orchestrator-hallucinated case).
    top_n_before = tools_state.fs_read(scan_id, keys.FS_FILE_TRIAGE_TOPN) or []
    top_n_ids    = (
        {p.get("arxiv_id") for p in top_n_before if isinstance(p, dict)}
        if isinstance(top_n_before, list) else set()
    )
    existing     = tools_state.fs_read(scan_id, path) is not None
    is_off_topn  = bool(top_n_ids) and (arxiv_id not in top_n_ids)
    is_retry     = existing or is_off_topn
    tools_state.fs_write(scan_id, path, payload)
    _mirror(scan_id, path, payload)
    try: runtime.llm_counter.service.set_phase("deep_read")
    except Exception: pass
    if is_retry:
        try:
            runtime.llm_counter.service.bump_retry_sync(scan_id, "deep_read")
        except Exception as e:
    ...
    try:
        runtime.service.set_extraction_sync(arxiv_id, payload)
    except Exception as e:
...
def write_synthesis_report(
    ...
    existing_report = tools_state.fs_read(scan_id, keys.FS_FILE_SYNTHESIS_REPORT)
    ...
    tools_state.fs_write(scan_id, keys.FS_FILE_SYNTHESIS_REPORT, payload)
    _mirror(scan_id, keys.FS_FILE_SYNTHESIS_REPORT, payload)
    try: runtime.llm_counter.service.set_phase("synthesis")
    except Exception: pass
    logger.info(
        f"[fs-tool] write_synthesis_report scan_id={scan_id} "
        f"themes={len(themes)} per_paper_themes={len(cleaned_ppt)} "
        f"path={keys.FS_FILE_SYNTHESIS_REPORT}"
    )
    _safe_emit(scan_id, "synthesis", f"{len(themes)} themes clustered")
    return (
        f"wrote synthesis report to {keys.FS_FILE_SYNTHESIS_REPORT} "
...
def write_digest(scan_id: str, digest_json: str) -> str:
    ...
    if tools_state.fs_read(scan_id, keys.FS_FILE_SYNTHESIS_REPORT) is None:
        msg = (
            ...
            f"`{keys.FS_FILE_SYNTHESIS_REPORT}` does not exist yet. "
    ...
    try:
        payload, repair_tag = domain.try_parse_with_repairs(digest_json)
        ...
        try:
            tools_state.fs_write(scan_id, debug_path, digest_json)
    ...
    existing = tools_state.fs_read(scan_id, keys.FS_FILE_DIGEST)
    if n_items == 0 and isinstance(existing, dict):
        ...
        if isinstance(existing_items, list) and len(existing_items) > 0:
            ...
            return (
                f"already wrote {keys.FS_FILE_DIGEST} with "
                f"{len(existing_items)} items; refusing empty overwrite"
            )

    tools_state.fs_write(scan_id, keys.FS_FILE_DIGEST, payload)
    _mirror(scan_id, keys.FS_FILE_DIGEST, payload)
    logger.info(
        f"[fs-tool] write_digest scan_id={scan_id} items={n_items} "
        f"path={keys.FS_FILE_DIGEST} ({log_msg})"
    )
    _safe_emit(scan_id, "report", f"{n_items} items in digest")
    return f"wrote final digest to {keys.FS_FILE_DIGEST} ({log_msg})"
