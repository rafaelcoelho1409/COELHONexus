...
def _build_year_filter(req: schemas.SearchInput) -> str | None:
...
def _build_headers() -> dict[str, str]:
    ...
    headers = {"User-Agent": config.S2.user_agent}
    api_key = os.environ.get(config.S2.api_key_env)
    if api_key:
...
async def search_s2(
    ...
    async with httpx.AsyncClient(timeout=config.S2.timeout_s) as client:
        resp = await client.get(
            f"{config.S2.base_url}/paper/search",
            params=params,
            headers=headers,
        )
