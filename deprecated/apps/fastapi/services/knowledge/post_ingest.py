...
# Per-section minimum size after splitting a monolith. Sections below this
# are heading-only stubs (e.g. an H2 with no body, or a single placeholder
# line) and add no semantic signal — they waste downstream embed budget +
# create noise micro-clusters in the planner. Tuning: LiteLLM 2026-05-15
# v3 baseline had min=8 bytes for the smallest survived section; 64 cuts
# heading-only stubs while preserving anything with even one prose sentence.
# See docs/KD-PLANNER-ANALYSIS-2026-05-15.md §2.1.
SPLIT_MIN_SECTION_BYTES = 64
...
async def split_monolith_if_needed(
    ...
    # Phase 1.5 — post-split cleanup (2026-05-15, KD-PLANNER-ANALYSIS §2.1):
    #
    # (a) Stub filter: drop sections whose body is below
    #     SPLIT_MIN_SECTION_BYTES. These are heading-only stubs that pass
    #     the splitter (markdown-it-py emits them as valid sections) but
    #     carry no prose, so they only waste downstream embed budget +
    #     create noise 1-3-file micro-clusters in the planner. The
    #     LiteLLM v3 baseline had min=8 bytes survive — the 64-byte cap
    #     keeps single-prose-sentence sections.
    #
    # (b) SHA256 content dedup: drop sections with identical body bytes.
    #     LiteLLM 2026-05-15 v3 produced 5+ ordinal copies of
    #     `demo-instance-https` (slugs 0053, 0136, 0147, 0157, 0167) all
    #     with byte-identical bodies — markdown-it-py was emitting the
    #     same content section per H2 heading whose body happened to be
    #     duplicated upstream. Catching it here saves 5+ embed calls per
    #     run AND prevents the dup ordinals from corrupting cluster
    #     centroids before `_dedup_chapter_files` (2.3) runs.
    #
    # Keep first occurrence on dedup — the lowest-ordinal slug preserves
    # document order for downstream alphabetical consumers.
    pre_filter = len(writes)
    stubs_dropped = sum(
        1 for _, b in writes
    ...
    if stubs_dropped or duplicates_dropped:
        logger.info(
            f"[post-ingest] split cleanup: dropped {stubs_dropped} stub "
            f"sections (<{SPLIT_MIN_SECTION_BYTES}B) + {duplicates_dropped} "
            f"duplicate-content sections; {len(writes)} survive "
            f"(was {pre_filter})"
        )
    # Defensive: if cleanup wiped everything, log loudly and let the
    # downstream planner raise its usual FileNotFoundError with a clear
    # message. We don't try to re-resurrect anything — the original
    # monolith has already been deleted by the time we get here.
    if not writes:
        logger.error(
            f"[post-ingest] split cleanup left ZERO surviving sections "
            f"(was {pre_filter}, all dropped as stubs or duplicates). "
            f"Original monolith {src_key} was already deleted; planner "
            f"will fail with empty corpus. Inspect upstream split logic."
        )
