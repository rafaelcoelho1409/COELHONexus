...
def run_knowledge_distiller(
    ...
    async def _run():
        ...
        # Observability — emit per-file `cache_restored` records so the
        # /studies/{id}/observability/ingestion page shows the corpus that
        # actually landed in study_root, not "0 records" like before. The
        # cache-restore path (server-side CopyObject) bypasses every tier
        # ingester so `record_url` was never firing on cache-hit studies.
        # See `docs/KD-PIPELINE-SUBSTEP-MAP-2026-05-15.md` (Stage 1.2).
        try:
            from services.knowledge.ingest_progress import IngestProgress
            progress = IngestProgress(study_id)
            try:
                await progress.start(
                    tier = cached_tier_used,
                    total = len(cached_manifest),
                )
                total_bytes = 0
                for i, entry in enumerate(cached_manifest, start = 1):
                    url = entry.get("url") or f"cache://{entry.get('slug', '')}"
                    bytes_fetched = entry.get("bytes") or 0
                    total_bytes += bytes_fetched
                    await progress.record_url(
                        url,
                        status = "cache_restored",
                        tier = entry.get("tier") or cache_hit.tier_used,
                        http_code = None,
                        fetch_ms = None,
                        bytes_fetched = bytes_fetched,
                        extracted_chars = bytes_fetched,
                    )
                    # update() is throttled (≤1/s) so 776 calls is cheap
                    await progress.update(current = i, last_url = url)
                await progress.record_post_ingest_summary(
                    tier = cached_tier_used,
                    input_files = len(cached_manifest),
                    input_bytes = total_bytes,
                    output_files = len(cached_manifest),
                    output_bytes = total_bytes,
                    was_split = False,
                    notes = (
                        f"restored from cache (cached_at="
                        f"{cache_hit.cached_at})"
                    ),
                )
            ...
            logger.warning(
                f"[KD:{study_id}] cache-restore observability emit failed: {e}"
            )
