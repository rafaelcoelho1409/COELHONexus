...
async def create_study(
    ...
    if cache_hit is None:
        # Celery task_id must be unique per chain step (a fresh uuid), but the
        # task-function `study_id` kwarg is the LOGICAL identifier used by
        # IngestProgress to key Redis writes (`coelhonexus:knowledge:ingest_*:{study_id}`).
        # Pass the user-facing study_id so `/studies/{id}/observability/ingestion`
        # and `/studies/{id}/stream` see this task's per-URL records under the
        # same key the caller is polling. Previously the two were the same uuid,
        # which silently dropped ingest progress for any cache-miss chain.
...
async def create_batch(
    ...
    for _, study in ordered_studies:
        ...
        if cache_hit is None:
            # See the matching note in /studies above: separate Celery task_id,
            # SAME logical study_id, so IngestProgress writes land where the
            # observability + SSE endpoints read.
            ingestion_task_id = str(uuid.uuid4())
            ingestion_sig = run_knowledge_ingestion.si(
                study_id = study_id,
                framework = joined_framework,
...
# =============================================================================
# GET /studies/{study_id}/observability/ingestion
# Per-URL stream of resolver + ingestion sub-step state for the FastHTML
# observability page (apps/fasthtml/routes/kd.py). Reads the Redis list
# written by IngestProgress.record_url() from each tier ingester, plus the
# throttled per-tier counter, and returns both as one snapshot. Polled every
# 2s by the page fragment.
# =============================================================================
@router.get("/studies/{study_id}/observability/ingestion")
async def get_ingestion_observability(
    study_id: str,
    request: Request):
    """
    Snapshot of ingestion sub-step state, for real-time visualization.
    ...
    Returns:
        {
            ...
            "header": {
                ...
                "current": N, "total": M,    # throttled per-tier counter
                "status": "idle|running|done|failed|downgrade",
                "last_url": ...,
                "updated_at": ...
            },
            "urls": [
                {
                    "url": ...,
                    "tier": ...,
                    "status": "success|http_error|timeout|extract_empty|fetch_error|downgrade",
                    "http_code": int|null,
                    "fetch_ms": int|null,
                    "bytes": int|null,
            ...
            "summary": {
                "total_recorded": int,
                "by_status": {success: N, http_error: N, ...},
                "by_tier": {sitemap: N, llms_txt: N, ...},
                "median_fetch_ms": int|null,
                "p95_fetch_ms": int|null,
                "total_bytes": int,
                "total_extracted_chars": int
            }
        }

    Returns 404 if the study record doesn't exist. Empty `urls`/`header=None`
    is normal before ingestion fires the first record.
    """
    record = await _load_study_record(request.app.state.redis_aio, study_id)
    if not record:
        raise HTTPException(status_code = 404, detail = "Study not found")

    from services.knowledge.ingest_progress import (
        read_progress, read_urls, read_post_ingest,
    )

    redis_aio = request.app.state.redis_aio
    header = await read_progress(redis_aio, study_id)
    urls = await read_urls(redis_aio, study_id)
    post_ingest = await read_post_ingest(redis_aio, study_id)
    ...
    for u in urls:
        ...
        if isinstance(fm, int):
            fetch_times.append(fm)
        b = u.get("bytes")
        if isinstance(b, int):
            total_bytes += b
        ec = u.get("extracted_chars")
        if isinstance(ec, int):
            total_extracted += ec

    median_fetch_ms = None
    p95_fetch_ms = None
    if fetch_times:
        fetch_times_sorted = sorted(fetch_times)
        n = len(fetch_times_sorted)
        median_fetch_ms = fetch_times_sorted[n // 2]
        p95_idx = max(0, min(n - 1, int(round(0.95 * (n - 1)))))
        p95_fetch_ms = fetch_times_sorted[p95_idx]

    return {
        "study_id": study_id,
        "study_root": record.get("study_root"),
        "header": header,
        "urls": urls,
        "summary": {
            "total_recorded": len(urls),
            "by_status": by_status,
            "by_tier": by_tier,
            "median_fetch_ms": median_fetch_ms,
            "p95_fetch_ms": p95_fetch_ms,
...
# =============================================================================
# POST /studies/{study_id}/observability/planner/replay/{shard_idx}
# Re-fire a single MAP shard's labeling call with the SAME input slugs but
# WHATEVER LLM the rotator picks now. Returns the new ShardLabels output
# for diff against the original record. Useful for non-determinism
# debugging — e.g., "did the model produce a different cluster name this
# time?" or "is the rotator picking a different deployment after the
# bandit learned something?". Best-effort; does not mutate the study.
# =============================================================================
@router.post("/studies/{study_id}/observability/planner/replay/{shard_idx}")
async def replay_planner_shard(
    study_id: str,
    shard_idx: int,
    request: Request):
    """
    Replay one MAP shard's LLM call. Reads the study_root from the Redis
    study record, re-derives the shard's slug list from the per-shard
    observability record (stored when the original shard ran), re-reads
    the slug contents from MinIO, fires SHARD_LABEL_PROMPT against the
    current `kd-all` rotator selection, and returns the new ShardLabels.

    Does NOT update the per-shard observability record — the original
    output is preserved so the page can show original vs replay diff.
    """
    from services.knowledge.planner_progress import read_planner_snapshot
    record = await _load_study_record(request.app.state.redis_aio, study_id)
    if not record:
        raise HTTPException(status_code = 404, detail = "Study not found")

    snapshot = await read_planner_snapshot(
        request.app.state.redis_aio, study_id,
    )
    shard_results = snapshot.get("shard_results") or []
    target = next(
        (s for s in shard_results if s.get("idx") == shard_idx),
        None,
    )
    if target is None:
        raise HTTPException(
            status_code = 404,
            detail = f"shard {shard_idx} not found in observability records",
        )

    # Collect this shard's input slugs from its recorded clusters
    shard_slugs: list[str] = []
    for c in (target.get("clusters") or []):
        shard_slugs.extend(c.get("file_slugs") or [])
    shard_slugs = list(dict.fromkeys(shard_slugs))  # dedup, preserve order
    if not shard_slugs:
        raise HTTPException(
            status_code = 400,
            detail = "shard record has no input slugs to replay",
        )

    # Re-read slug bodies from MinIO
    storage = request.app.state.study_storage
    study_root = record["study_root"]
    bodies: list[tuple[str, str]] = []
    for slug in shard_slugs:
        try:
            body = await storage.read_text(
                f"{study_root}/research/raw/{slug}.md",
            )
            bodies.append((slug, body))
        except Exception as e:
            logger.warning(f"[replay][shard {shard_idx}] missing {slug}: {e}")
    if not bodies:
        raise HTTPException(
            status_code = 500,
            detail = "no slug bodies could be re-read from MinIO",
        )

    # Build a minimal `_label_shard`-equivalent prompt + LLM call
    from services.llm_chain import build_llm_fallback_chain
    from schemas.knowledge.prompts import SHARD_LABEL_PROMPT
    from schemas.knowledge.agents import ShardLabels
    from graphs.knowledge.helpers import _build_corpus_summary

    llm = build_llm_fallback_chain()
    shard_summary = _build_corpus_summary(bodies)

    label_chain = SHARD_LABEL_PROMPT | llm.with_structured_output(
        ShardLabels, method = "function_calling", include_raw = False,
    )
    try:
        replayed: ShardLabels = await label_chain.ainvoke({
        ...
        raise HTTPException(
            status_code = 500,
            detail = f"replay LLM call failed: {type(e).__name__}: {e}",
        )

    # Filter hallucinated slugs (LLM may invent slugs not in input)
    valid = set(shard_slugs)
    for c in replayed.clusters:
        c.file_slugs = [s for s in c.file_slugs if s in valid]

    return {
        "study_id": study_id,
        "shard_idx": shard_idx,
        "input_slug_count": len(shard_slugs),
        "original": target,
        "replay": {
            "n_clusters": len(replayed.clusters),
            "n_unused": len(replayed.unused_shard_slugs or []),
            "clusters": [
                {
                    "name": c.cluster_name,
...
# =============================================================================
# GET /studies/{study_id}/observability/planner
# Snapshot of every planner sub-step state (Stage 2 of the per-node viz
# stack). Reads all `coelhonexus:knowledge:planner:*:{study_id}` keys
# written by services/knowledge/planner_progress.py. See
# docs/KD-PIPELINE-SUBSTEP-MAP-2026-05-15.md.
# =============================================================================
@router.get("/studies/{study_id}/observability/planner")
async def get_planner_observability(
    study_id: str,
    request: Request):
    """
    Snapshot of planner sub-step state for the FastHTML observability page.

    Returns the full per-substep dict from `read_planner_snapshot()`:
        status, corpus_load, off_topic, dedup, cache, shards,
        shard_results[], reduce_embed, reduce_umap, reduce_k,
        reduce_kmeans, reduce_thin, reduce_split, chapter_coherence,
        validation, coverage.
