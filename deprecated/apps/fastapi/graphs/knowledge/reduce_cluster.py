...
async def embed_and_cluster_reduce(
    ...
    progress: "Any | None" = None,  # PlannerProgress, kept as Any to avoid circular import
) -> ChapterPlanList:
    """
    ...
    logger.info(
        ...
        f"in {embed_ms / 1000:.2f}s via {embed_provider}"
    )
    if progress is not None:
        try:
            await progress.set_phase("reduce")
            await progress.record_reduce_embed(
    ...
    pca_in_dim_obs: int | None = None
    pca_out_dim_obs: int | None = None
    pca_var_obs: float | None = None
    if vectors.shape[1] > _PCA_COMPONENTS and n_clusters > _PCA_COMPONENTS:
        ...
        try:
            ...
            pca_in_dim_obs = int(vectors.shape[1])
            pca_out_dim_obs = int(vectors_for_umap.shape[1])
            pca_var_obs = variance_kept
            logger.info(
    ...
    t0 = time.time()
    umap_n_neighbors_eff = min(_UMAP_N_NEIGHBORS, n_clusters - 1)
    try:
        from umap import UMAP
        umap_model = UMAP(
            n_neighbors=umap_n_neighbors_eff,
        ...
        umap_ms = int((time.time() - t0) * 1000)
        logger.info(
            f"[reduce-cluster] UMAP {vectors_for_umap.shape[1]}d → "
            f"{vectors_reduced.shape[1]}d in {umap_ms / 1000:.2f}s "
            f"(n_neighbors={umap_n_neighbors_eff}, "
    ...
    if progress is not None:
        try:
            await progress.record_reduce_umap(
                pca_in_dim = pca_in_dim_obs,
                pca_out_dim = pca_out_dim_obs,
                pca_explained_variance = pca_var_obs,
                umap_in_dim = int(vectors_for_umap.shape[1]),
                umap_out_dim = int(vectors_reduced.shape[1]),
                n_neighbors = umap_n_neighbors_eff,
    ...
    if progress is not None:
        try:
            await progress.record_reduce_k_selection(
                k_meta = k_meta,
                k_volume = k_volume,
                k_target = k_target,
                final_k = k_target,  # may shift after sweep below
                clamp_min = _MIN_CHAPTERS,
                clamp_max = _MAX_CHAPTERS,
            )
        except Exception:
            pass
    ...
    kmeans_ms = int((time.time() - t0) * 1000)
    logger.info(
        f"[reduce-cluster] k={best_k} CH={best_ch:.1f} silhouette={best_sil:.3f} "
        f"sizes=[{sizes_str}] sweep=[{sweep_str}] in {kmeans_ms / 1000:.2f}s"
    )
    if progress is not None:
        try:
            await progress.record_reduce_kmeans(
                sweep = [
                    {"k": int(k), "ch": float(ch), "silhouette": float(sil)}
                    for k, ch, sil in sweep_results
                ],
                best_k = int(best_k),
                cluster_sizes = [int(size_counter[i]) for i in range(best_k)],
                size_min = 0,  # per-k; aggregate not tracked here
                size_max = 0,
                elapsed_ms = kmeans_ms,
            )
        except Exception:
            pass
    ...
    if progress is not None:
        try:
            # merged_pairs items are (thin_mid, fat_mid, files_moved). The
            # cosine sim is computed inline above but not stored — surface
            # only what's available.
            await progress.record_reduce_thin_merges(
                threshold_files = _THIN_CHAPTER_FILE_THRESHOLD,
                merges = [
                    {
                        "thin_mid": int(t),
                        "fat_mid": int(f),
    ...
    if progress is not None:
        try:
            await progress.record_reduce_splits(
                file_cap_fraction = float(_META_MAX_FILE_FRACTION),
                splits = [
                    {
                        "orig_mid": int(orig),
                        "files_before": int(before),
                        "sub_buckets": [
                            {"files": int(n), "coherence": None}
                            for n in after_list
                        ],
                        "sub_k": len(after_list),
                    }
                    for orig, before, after_list in split_log
                ],
                elapsed_ms = None,
            )
        except Exception:
