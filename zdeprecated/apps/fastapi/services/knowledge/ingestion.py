...
async def ingest_framework_docs(
    ...
    # Observability: record the input → output expansion from the split so
    # the /studies/{id}/observability/ingestion page can show "1 URL fetched
    # → N files in MinIO" honestly. Without this the per-URL table looks like
    # the corpus is tiny on Tier 1, when reality is 700+ split sections.
    from services.knowledge.ingest_progress import IngestProgress
    post_progress = IngestProgress(cfg.study_id)
    try:
        await post_progress.record_post_ingest_summary(
            tier = result.tier_used,
            input_files = len(result.manifest),
            input_bytes = sum(e.bytes for e in result.manifest),
            output_files = len(new_manifest),
            output_bytes = sum(e.bytes for e in new_manifest),
            was_split = len(new_manifest) != len(result.manifest),
            notes = (
                "monolith split on H1/H2 boundaries"
                if len(new_manifest) != len(result.manifest)
                else "no split (multi-file manifest or below threshold)"
            ),
        )
    finally:
        await post_progress.close()
