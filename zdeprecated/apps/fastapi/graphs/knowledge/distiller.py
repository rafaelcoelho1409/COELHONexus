...
class KnowledgeDistillerGraph:
    ...
    async def planner(
        ...
        # Observability — Stage 2 of the per-node viz stack
        # (docs/KD-PIPELINE-SUBSTEP-MAP-2026-05-15.md). PlannerProgress writes
        # per-substep snapshots to Redis at `coelhonexus:knowledge:planner:*`,
        # consumed by /api/v1/knowledge/studies/{id}/observability/planner.
        # Every pp.* call is wrapped in try/except — observability failure
        # MUST NOT break the planner. pp is left open (Redis client GC'd
        # by IngestProgress pattern on exception, no explicit close needed).
        from services.knowledge.planner_progress import PlannerProgress
        pp = PlannerProgress(study_id)

        try: await pp.set_phase("corpus_load")
        ...
        load_ms = int((_ptime.time() - t0) * 1000)
        if not entries:
            raise FileNotFoundError(f"research/raw/ is empty at prefix {study_root!r}")
        try:
            sizes = sorted(len(b) for _, b in entries)
            await pp.record_corpus_load(
        ...
        try: await pp.set_phase("off_topic")
        except Exception: pass
        # Pre-MAP semantic off-topic filter (2026-05-09).
        ...
        entries, off_diag = await _filter_off_topic_files(
            entries, framework=framework, return_diag=True,
        )
        filter_ms = int((_ptime.time() - t0) * 1000)
        ...
        try:
            await pp.record_off_topic(
                framework = framework,
                threshold = _OFF_TOPIC_THRESHOLD,
                kept = len(entries),
                dropped = filtered,
                per_file_cosines = off_diag.get("per_file_cosines") or [],
                domain_coherence = off_diag.get("domain_coherence"),
                embedding_provider = off_diag.get("provider"),
                elapsed_ms = filter_ms,
            )
        except Exception:
            pass

        try: await pp.set_phase("dedup")
        except Exception: pass
        # Tier 2 #6 — code-aware near-dup filter.
        ...
        entries, dedup_diag = _dedup_chapter_files(entries, return_diag=True)
        dedup_ms = int((_ptime.time() - t0) * 1000)
        ...
        try:
            await pp.record_dedup(
                threshold = _MINHASH_JACCARD_THRESHOLD,
                pairs_checked = dedup_diag.get("pairs_checked", 0),
                dropped = deduped,
                dedup_log = dedup_diag.get("dedup_log") or [],
                elapsed_ms = dedup_ms,
            )
        except Exception:
            pass
        ...
        try: await pp.set_phase("cache_lookup")
        except Exception: pass
        ...
        try:
            await pp.record_cache_lookup(
                manifest_hash = manifest_hash,
                hit = hit is not None,
                cached_at = (
                    hit.cached_at.isoformat()
                    if hit is not None and hasattr(hit.cached_at, "isoformat")
                    else (str(hit.cached_at) if hit is not None else None)
                ),
                age_seconds = None,
            )
        except Exception:
        ...
        try:
            await pp.set_phase("shard_map")
            await pp.record_shards(
                total_shards = len(shards),
                shard_sizes = [len(s) for s in shards],
                shard_size_cap = _SHARD_SIZE,
            )
        except Exception:
            pass
        ...
        async def _label_shard_bounded(shard, shard_idx):
            async with MAP_SHARD_SEMAPHORE:
                t_shard_start = _asyncio.get_event_loop().time()
                try:
                    result = await _asyncio.wait_for(
                        _label_shard(shard, shard_idx),
                        timeout = _MAP_SHARD_TIMEOUT_SECONDS,
                    )
                    # Observability — emit per-shard record. Path is best-
                    # effort detection: cluster name prefixed "Shard N (..."
                    # indicates fallback or catchall; otherwise strict_json.
                    elapsed_ms = int(
                        (_asyncio.get_event_loop().time() - t_shard_start) * 1000
                    )
                    cn = result.clusters[0].cluster_name if result.clusters else ""
                    if "(timed out)" in cn:
                        path_obs = "timeout"
                    elif "(unlabeled)" in cn or "(parse-fallback)" in cn:
                        path_obs = "catchall"
                    else:
                        # strict vs fallback indistinguishable from output;
                        # default to strict_json. Per-call telemetry would
                        # need to thread through _label_shard return.
                        path_obs = "strict_json"
                    try:
                        await pp.record_shard_result(
                            shard_idx,
                            path = path_obs,
                            n_input_slugs = len(shard),
                            n_clusters = len(result.clusters),
                            n_unused = len(result.unused_shard_slugs or []),
                            clusters = [
                                {
                                    "name": c.cluster_name,
                                    "description": c.description,
                                    "file_slugs": list(c.file_slugs),
                                    "n_files": len(c.file_slugs),
                                }
                    ...
                    try:
                        await pp.record_shard_result(
                            ...
                            n_unused = 0,
                            clusters = [{
                                "name": timeout_result.clusters[0].cluster_name,
                                "description": timeout_result.clusters[0].description,
                                "file_slugs": shard_slugs,
                                "n_files": len(shard_slugs),
                            }],
                            timeout_ms = _MAP_SHARD_TIMEOUT_SECONDS * 1000,
                            elapsed_ms = elapsed_ms,
                            error_msg = f"timeout after {_MAP_SHARD_TIMEOUT_SECONDS}s",
                        )
                    except Exception:
                        pass
        ...
        map_path_used = "llm_map"  # "llm_map" | "classical_map" | "global_classical_map"
        if _use_global_map():
            ...
            map_path_used = "global_classical_map"
        ...
        # Observability — emit per-shard records for the CLASSICAL paths too.
        # `_label_shard_bounded` records its own per-shard data inline (so the
        # page can update live as each shard finishes). The classical paths
        # finish in a single batch — record their results at the call site
        # AFTER the gather completes, so all three MAP paths surface
        # equivalently on the observability page.
        if map_path_used != "llm_map":
            for sidx, sr in enumerate(shard_results, start = 1):
                try:
                    await pp.record_shard_result(
                        sidx,
                        path = map_path_used,
                        n_input_slugs = sum(len(c.file_slugs) for c in sr.clusters),
                        n_clusters = len(sr.clusters),
                        n_unused = len(sr.unused_shard_slugs or []),
                        clusters = [
                            {
                                "name": c.cluster_name,
                                "description": c.description,
                        ...
                        elapsed_ms = None,  # not measured per-shard in classical paths
                    )
                except Exception:
                    pass
        ...
        # ─── Ch02 mis-routing detector ────────────────────────────────────
        # For each chapter the planner emitted, compute the coherence
        # between the chapter TITLE and the CONTENT of its assigned files:
        #
        #   coherence_score = mean( cos(title_emb, file_emb)
        #                           for file in chapter.assigned_files )
        #
        # Low coherence (< 0.35) means the chapter title doesn't match its
        # files — the Ch02-class defect we saw on Docker where title was
        # "Docker Account Management" but body was Linux install procedures.
        #
        # Cost: one extra embedding batch of N+M items (M chapter titles,
        # N file snippets reused from the off-topic filter step). Per memory
        # `feedback_kd_quality_over_speed.md`, this trade is correct.
        try:
            import time as _ctime
            import numpy as _np
            from services.knowledge.embeddings import embed_texts as _embed_texts
            from graphs.knowledge.helpers import _OFF_TOPIC_SNIPPET_CHARS as _SNIP
            t0 = _ctime.time()
            slug_to_body = {slug: body for slug, body in entries}
            unique_slugs: list[str] = []
            seen_slugs: set[str] = set()
            for ch in plan.chapters:
                for s in ch.assigned_files:
                    if s in slug_to_body and s not in seen_slugs:
            ...
            file_texts = [
                (slug_to_body.get(s) or "").strip()[:_SNIP] or " "
                for s in unique_slugs
            ]
            all_texts = title_texts + file_texts
            if all_texts:
                vecs_list, coh_provider = await _embed_texts(all_texts)
                vecs = _np.asarray(vecs_list, dtype=_np.float32)
                titles_v = vecs[: len(title_texts)]
                files_v = vecs[len(title_texts):]
                norms_t = _np.linalg.norm(titles_v, axis=1)
                norms_f = _np.linalg.norm(files_v, axis=1)
                slug_to_idx = {s: i for i, s in enumerate(unique_slugs)}
                chapters_out: list[dict] = []
                for ti, ch in enumerate(plan.chapters):
                    tv = titles_v[ti]
                    tn = float(norms_t[ti])
                    per_file_cos: list[tuple[str, float]] = []
                    for s in ch.assigned_files:
                        fi = slug_to_idx.get(s)
                        if fi is None:
                            continue
                        fv = files_v[fi]
                        fn = float(norms_f[fi])
                        denom = max(tn * fn, 1e-12)
                        per_file_cos.append((s, float((tv @ fv) / denom)))
                    if per_file_cos:
                        mean_cos = sum(c for _, c in per_file_cos) / len(per_file_cos)
                    else:
                        mean_cos = 0.0
                    low_cos = sorted(per_file_cos, key=lambda kv: kv[1])[:5]
                    chapters_out.append({
                        "number": ch.number,
                        "title": ch.title,
                        "n_files": len(ch.assigned_files),
                        "coherence_score": float(mean_cos),
                        "low_coherence_files": [
                            {"slug": s, "cosine": float(c)} for s, c in low_cos
                        ],
                    })
                await pp.record_chapter_coherence(
                    chapters = chapters_out,
                    threshold_red = 0.35,
                    threshold_yellow = 0.50,
                    embedding_provider = coh_provider,
                    elapsed_ms = int((_ctime.time() - t0) * 1000),
                )
                # Loud log for the most-suspect chapter so operators see
                # mis-routing in the Celery log even before opening the UI.
                worst = min(chapters_out, key=lambda c: c["coherence_score"])
                if worst["coherence_score"] < 0.35:
                    logger.warning(
                        f"[planner] Ch{worst['number']} '{worst['title']}' has "
                        f"low title-coherence ({worst['coherence_score']:.3f}); "
                        f"likely mis-routed assigned_files — inspect "
                        f"/kd/studies/.../observability/planner"
                    )
        except Exception as _e:
            logger.warning(f"[planner] chapter-coherence diagnostic failed: {_e}")

        try: await pp.set_phase("validate")
        except Exception: pass
        ...
        # Observability — validation + coverage repair summary
        try:
            await pp.record_validation(
                warnings = warnings_collected,
                has_duplicates = any("duplicate" in w.lower() for w in warnings_collected),
                orphan_count = len(orphans),
                hallucinated_count = len(hallucinated),
                drop_rate = (len(orphans) / max(1, len(available))),
                is_valid = not warnings_collected,
            )
            await pp.record_coverage_repair(
                orphans_added = len(orphans),
                orphans_examples = sorted(orphans)[:5],
                hallucinated_dropped = len(hallucinated),
                chapters_affected = {},
            )
        except Exception:
            pass
