...
async def _filter_off_topic_files(
    ...
    Args:
        ...
        return_diag: when True, return (kept, diag_dict) so the planner
                    observability page can surface per-file cosines +
                    embedding provider + domain coherence. False preserves
                    the original signature for non-planner callers.

    Returns:
        - If `return_diag=False` (default): entries minus off-topic docs,
          preserving input order. On any embedding failure, returns input
          unchanged.
        - If `return_diag=True`: (kept_entries, diag) where diag is a dict
          with keys: per_file_cosines (list of (slug, cos, kept_bool)),
          provider, domain_coherence, error_msg (None on success).
    """
    diag: dict = {
        "per_file_cosines": [],
        "provider": None,
        "domain_coherence": None,
        "error_msg": None,
    }
    if not entries:
        return (entries, diag) if return_diag else entries
    try:
        import numpy as np
        ...
        for (slug, body), cos, vec in zip(entries, cosines, docs):
            cf = float(cos)
            keep = cf >= threshold
            per_file.append((slug, cf, keep))
            if keep:
        ...
        # Domain coherence: how tight is the kept set vs its own centroid?
        # Low coherence (e.g. < 0.5) means the kept corpus is itself
        # scattered; the planner is about to chapter-cluster a mess.
        domain_coherence: float | None = None
        if len(kept_vectors) >= 2:
            kv = np.asarray(kept_vectors, dtype=np.float32)
            centroid = kv.mean(axis=0)
            centroid_norm = float(np.linalg.norm(centroid))
            if centroid_norm > 0:
                kv_norms = np.linalg.norm(kv, axis=1)
                centroid_cos = (kv @ centroid) / np.maximum(
                    kv_norms * centroid_norm, 1e-12,
                )
                domain_coherence = float(centroid_cos.mean())
        if dropped > 0:
            logger.info(
        ...
        return (kept, diag) if return_diag else kept
    except Exception as e:
        err = f"{type(e).__name__}: {e}"
        logger.warning(
            f"[off-topic-filter] failed ({err}); keeping all entries"
        )
        diag["error_msg"] = err
        return (entries, diag) if return_diag else entries
...
def _dedup_chapter_files(
    ...
    Args:
        entries:         list of (slug, body) tuples.
        jaccard_threshold: prose Jaccard floor for considering a pair a dup
                         (default 0.85). Code-block sets must also match
                         exactly — one differing fence kills the dup verdict.
        return_diag:     when True, return (kept, diag_dict) so the planner
                         observability page can surface per-pair Jaccard
                         scores and the kept-vs-dropped decisions.

    Returns:
        - Default: filtered entries in original order. On exception, input
          unchanged.
        - `return_diag=True`: (filtered, diag) where diag has:
            pairs_checked: int
            dropped: int
            dedup_log: list[{slug_kept, slug_dropped, jaccard, code_match,
                             len_kept, len_dropped}]  (capped at 50)
            error_msg: str | None
    """
    diag: dict = {
        "pairs_checked": 0,
    ...
    try:
        ...
        for i in range(len(entries)):
            ...
            for j in range(i + 1, len(entries)):
                ...
                if pi_len >= pj_len:
                    ...
                    if len(dedup_log) < 50:
                        dedup_log.append({
                            "slug_kept": entries[i][0],
                            "slug_dropped": entries[j][0],
                    ...
                    if len(dedup_log) < 50:
                        dedup_log.append({
                            "slug_kept": entries[j][0],
                            "slug_dropped": entries[i][0],
                            "jaccard": float(jaccard),
                            "code_match": True,
                            "len_kept": pj_len,
                            "len_dropped": pi_len,
        ...
        err = f"{type(e).__name__}: {e}"
        logger.warning(f"[dedup] failed ({err}); keeping all entries")
        diag["error_msg"] = err
        return (entries, diag) if return_diag else entries
